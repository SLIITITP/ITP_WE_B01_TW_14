...
//register vehicle to system
router.post("/registerVehicle", auth, vehicleuploads.single("vehicleImg"),async (req, res) => {
  //console.log(req.body);
  const file = req.file.filename
  console.log(file);
  const { registerNo, brand, model, vehicleType, vehicleColor, manufactureYear, fuelType, vehicleStatus, chassisNo, LicenceExpiredDate, InsuranceExpiredDate } = req.body;
  ...
  //check all the missing fields
  if (!registerNo || !brand || !model || !vehicleType || !vehicleColor || !manufactureYear || !fuelType || !file || !vehicleStatus || !chassisNo || !LicenceExpiredDate || !InsuranceExpiredDate) {
      res.status(422).json({error : `Please enter all the required fields`});
  }
  ...
  try {
      ...
      if (prevehicle) {
          ...
          const addvehicle = new vehicles({
              registerNo, brand, model, vehicleType, vehicleColor, manufactureYear, fuelType, vehicleImg: file, vehicleStatus, chassisNo, LicenceExpiredDate, InsuranceExpiredDate, datecreated
          });
...
//get vehicle from data
router.get("/getvehidata", async (req, res) => {
  try {
    const vehiclesdata = await vehicles.find();
...
router.get("/getdata",auth, async (req, res) => {
  ...
  const query = {
      ...
      fuelType:fuel === "All"?{$regex:"", $options:"i"} : {$regex:fuel, $options:"i"},
      //vehicleStatus:status ==="All"?{$regex:"", $options:"i"} : {$regex:status, $options:"i"}
  }
  ...
  try {
      const vehicledata = await vehicles.find(query);
      res.status(201).json(vehicledata);
      //console.log(vehicledata);
  } catch (error) {
      res.status(422).json(error);
...
//update vehicle data
router.patch("/updatevehicle/:id", vehicleuploads.single("vehicleImg"),auth, async (req, res) => {
  try {
      const { id } = req.params;
      const { registerNo, brand, model, vehicleType, vehicleColor, manufactureYear, fuelType, vehicleStatus, chassisNo, LicenceExpiredDate, InsuranceExpiredDate } = req.body;
      //const file = req.file ? req.file.filename : vehicleImg1
      ...
      const updatedvehicle = await vehicles.findByIdAndUpdate({ _id:id }, {
          registerNo, brand, model, vehicleType, vehicleColor, manufactureYear, fuelType, vehicleStatus, chassisNo, LicenceExpiredDate, InsuranceExpiredDate, dateUpdated
      }, {
          new:true
      });
